...
describe('structural rejection (Zod phase)', () => {
  ...
  it('rejects a misspelled field key (strict objects catch typos)', () => {
    ...
    // Introduce a typo'd property on the first field of the contact-and-address step (index 7).
    (asStep(bad.steps[7]).fields[0] as unknown as Record<string, unknown>).requried = true;
    const result = validateWizardConfig(bad);
    expect(result.ok).toBe(false);
  });
  ...
  it('rejects a field with an empty label', () => {
    const bad = clone(fencingWizardConfig);
    asStep(bad.steps[7]).fields[0].label = '';
    const result = validateWizardConfig(bad);
    expect(result.ok).toBe(false);
  });
...
describe('semantic rejection (cross-reference phase)', () => {
  it('rejects a dangling condition fieldId reference', () => {
    const bad = clone(fencingWizardConfig);
    asStep(bad.steps[7]).fields[0].condition = {
      operator: 'equals',
  ...
  it('rejects duplicate field ids', () => {
    const bad = clone(fencingWizardConfig);
    // contact-and-address step (index 7) has: contact_name[0], contact_phone[1], contact_email[2], full_address[3]
    asStep(bad.steps[7]).fields[1].id = asStep(bad.steps[7]).fields[0].id;
    const result = validateWizardConfig(bad);
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it('rejects a choice field with no options', () => {
    const bad = clone(fencingWizardConfig);
    // extras step (index 4), fields[0] = include_gate (checkbox with options)
    delete (asStep(bad.steps[4]).fields[0] as { options?: unknown }).options;
    const result = validateWizardConfig(bad);
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it('rejects a non-choice field that defines options', () => {
    ...
    (asStep(bad.steps[7]).fields[0] as { options?: unknown }).options = [
      { value: 'x', label: 'X' },
    ];
    const result = validateWizardConfig(bad);
    expect(result.ok).toBe(false);
